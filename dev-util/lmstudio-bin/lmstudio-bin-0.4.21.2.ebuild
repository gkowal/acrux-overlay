...
src_install() {
	...
	doicon -s 512 usr/share/icons/hicolor/0x0/apps/lm-studio.png

	# Adjust executable path and remove invalid category in desktop entry, then install it
	sed -i 's|Exec=/opt/LM-Studio/lm-studio|Exec=lm-studio|g' usr/share/applications/lm-studio.desktop || die
	sed -i 's|Exec=AppRun|Exec=lm-studio|g' usr/share/applications/lm-studio.desktop || die
	sed -i '/^category=/d' usr/share/applications/lm-studio.desktop || die
