...
src_install() {
	...
	# Remove fragile prebuilt Qt theme shims that crash on Gentoo Qt 6.11
...
EOF
	...
	if [ -f "${WORKDIR}/etc/apparmor.d/chatgpt" ]; then
		...
		sed 's#/usr/lib/chatgpt#/opt/chatgpt#g' "${WORKDIR}/etc/apparmor.d/chatgpt" > "${D}/etc/apparmor.d/chatgpt" || die
	fi
