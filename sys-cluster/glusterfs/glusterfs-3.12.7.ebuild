...
IUSE="bd-xlator crypt-xlator debug emacs +fuse georeplication glupy infiniband libtirpc qemu-block rsyslog static-libs +syslog systemd systemtap test +tiering vim-syntax +xml"
...
PATCHES=(
	...
	"${FILESDIR}/${PN}-3.12-silent_rules.patch"
	"${FILESDIR}/${PN}-3.12-libtirpc.patch"
)
...
src_install() {
	...
	newinitd "${FILESDIR}/${PN}d.initd" glusterfsd
	newinitd "${FILESDIR}/glusterd.initd" glusterd
	newconfd "${FILESDIR}/${PN}.confd" glusterfsd

	if use systemd; then
		systemd_dounit "${FILESDIR}/glusterd.service"
	fi
