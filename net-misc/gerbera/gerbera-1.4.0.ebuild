...
LICENSE="GPL-2"
SLOT="0"
IUSE="curl debug +exif exiv2 +ffmpeg ffmpegthumbnailer +javascript lastfm libav +magic +matroska mysql protocol-extensions systemd +taglib"

DEPEND="
	!!net-misc/mediatomb
	>=net-libs/libupnp-1.10.1[ipv6,reuseaddr]
	>=dev-db/sqlite-3
	dev-libs/expat
...
src_install() {
	...
	insinto /etc/${PN}
	newins "${FILESDIR}/${PN}.config" config.xml
